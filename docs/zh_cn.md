...
![arcgis-map-event](https://img.shields.io/badge/arcgis--map--event-v1.0.4-%23C50008?logo=npm)
...
| `objectIdField` | 唯一objectId字段的名称。默认为`objectId` |
...
+ Q：要素图形无法触发事件？
+ A：你可以给图形的 `attributes`属性添加一个唯一的 `arcgis_map_event_id`。设置唯一的objectId用于帮助查找图形。
    ```js
    new Graphic({
        geometry,
        attributes: {
            arcgis_map_event_id: "unique id"
        }
    });
    ```
